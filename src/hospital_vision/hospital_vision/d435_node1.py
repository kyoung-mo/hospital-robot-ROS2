...
class D435Node(Node):
        ...
        for room in ["101", "102"]:
            ...
            if not np.isnan(self.depths[depth_key]) and self.depths[depth_key] < 750:
                if prev != "FULL":
                    ...
                    self.get_logger().warn(f"ROOM {room} WASTE FULL!")
    ...
    def check_fall(self):
        ...
        if not np.isnan(bed1) and bed1 > FALL_THRESHOLD:
            ...
            msg = String()
            msg.data = "101" # 친구 C++ 데이터용
            self.fall_pub.publish(msg)
            if prev1 != "SUSPECTED": self.get_logger().warn("🚨 101 FALL 감지!")
        else:
            self.fall_status_r1 = "NONE"
        ...
        if not np.isnan(bed2) and bed2 > FALL_THRESHOLD:
            ...
            msg = String()
            msg.data = "102" # 친구 C++ 데이터용
            self.fall_pub.publish(msg)
            if prev2 != "SUSPECTED": self.get_logger().warn("🚨 102 FALL 감지!")
        else:
            self.fall_status_r2 = "NONE"

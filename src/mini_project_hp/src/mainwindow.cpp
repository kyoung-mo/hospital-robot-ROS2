...
void MainWindow::handleBattery(int id, float p) {
    qDebug() << "--- UI: Battery Signal Received --- Robot:" << id << "Raw:" << p;
    // percentage가 0.85와 같이 들어오는지, 85.0과 같이 들어오는지 확인이 필요합니다.
    int val = 0;

    if (p <= 1.0f && p >= 0.0f) {
        // 0.0 ~ 1.0 범위로 들어올 경우 (표준 BatteryState)
        val = static_cast<int>(p * 100.0f);
    } else {
        // 56.65와 같이 이미 퍼센트 단위로 들어올 경우
        val = static_cast<int>(p);
    }
    // 범위 제한 (ProgressBar 에러 방지)
    val = std::max(0, std::min(100, val));

    // 2. UI 업데이트
    if (id == 1) {
        r1BattBar->setValue(val);
        r1Status->setText(QString("상태: 온라인 (%1%)").arg(val));
    } else if (id == 2) {
        r2BattBar->setValue(val);
        r2Status->setText(QString("상태: 온라인 (%1%)").arg(val));
    }

    // 3. 디버그 로그 (소수점까지 확인하기 위해 float p를 직접 출력)
    qDebug() << "Battery Update Success -> Robot ID:" << id << " Value:" << val;
}
